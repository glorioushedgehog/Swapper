...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname swapper) (read-case-sensitive #t) (teachpacks ((lib "image.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t constructor repeating-decimal #t #t none #f ((lib "image.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp")) #f)))
; the title of the game
(define TITLE "VIDEO GAME")
...
(define ENEMYCOLOR1 'Navy)
; the color of homing enemies
(define ENEMYCOLOR2 'Maroon)
; the color of the health bar
(define HEALTHBARCOLOR 'crimson)
...
(define (welcome-screen ws)
  (underlay/xy
   ...
   (above (text TITLE 220 TEXTCOLOR)
          ...
          (text (string-append "movement: " UP LEFT DOWN RIGHT) 40 TEXTCOLOR)
          (text "aim with the mouse" 40 TEXTCOLOR)
          (text "click to fire" 40 TEXTCOLOR)
          (text (string-append "press " (if (string=? SWAP " ") "space" SWAP) " if you want") 40 TEXTCOLOR)
          (text "press a key to start" 40 TEXTCOLOR))))
; Worldstate -> Image
...
(define (pause-screen ws)
  (underlay/xy
   ...
   (above (text TITLE 220 TEXTCOLOR)
          (square 100 'solid (color 255 255 255 0))
          (text "is paused" 40 TEXTCOLOR)
          (text "press a key to resume" 40 TEXTCOLOR))))
; Worldstate -> Image
...
(define (death-screen ws)
  (underlay/xy
   ...
   200
   100
   (above (text "FAILURE" 255 TEXTCOLOR)
          (square 100 'solid (color 255 255 255 0))
          (text (string-append "total kills: " (number->string (WS-kills ws)) " !!") 40 TEXTCOLOR)
          (square 100 'solid (color 255 255 255 0))
          (text "press a key to play again" 40 TEXTCOLOR))))
; Worldstate -> Image
; Draws the game
(define (draw-game ws)
